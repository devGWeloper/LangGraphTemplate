...
>    cd ..
>    ```
> 3. `notepad .env` 로 열어 아래 값을 채우고 저장하세요.
>    ```
>    LLM_BASE_URL=<값>
>    LLM_API_KEY=<값>
>    LLM_MODEL=<값>
...
조원은 `copy .env.example .env` 로 만든 파일에 값만 붙여넣으면 됩니다.

> 💡 조원이 메모장 "새로 만들기 → 저장"으로 `.env` 를 만들면 윈도우 확장자 숨김 때문에
> `.env.txt` 가 만들어지는 사고가 잦습니다. 꼭 `copy .env.example .env` 를 쓰라고 안내하세요.
> 확인은 `dir .env*` 로 합니다.
...
문서 맨 위의 **주제 선정 회의**, 맨 끝의 **산출물 정리 회의** 표는 배점 항목은 아니지만,
조원이 실제로 모여서 주제를 정하고 문서를 함께 마무리했는지 확인하는 용도입니다.
일시·장소·참석자가 비어 있거나 "어떻게 진행했는지"가 한 줄짜리 형식적인 문장이면
채점 전에 해당 조에 다시 채워달라고 요청하세요.
...
- **기획 산출물 대신 회의 기록 두 개(맨 위 "주제 선정 회의", 맨 끝 "산출물 정리 회의")만 둡니다.**
  기획서 형식의 문서를 요구하지 않되, 시작과 마무리 시점의 일시·장소·참석자를 남기게 해서
  조원이 실제로 오프라인에서 함께 진행했는지 확인할 수 있게 한 것입니다.
