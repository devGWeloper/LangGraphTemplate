...
| messages | list[dict] | 이전 대화 이력 (공용 제공). `_ask(..., history=state["messages"])` 로 넘겨 멀티턴을 만듭니다 |
| answer | str | 최종 답변 (공용 제공) |
| intent | str | 이번 발화가 `plan`(새로 짜달라)인지 `followup`(이어지는 질문)인지 |
...
프롬프트에 통째로 넘길 수 있기 때문입니다. 반대로 `intent`와 `missing`은 분기 판단에만 쓰이는 값이라
...
| 노드 | 역할 | 읽는 필드 | 쓰는 필드 |
|---|---|---|---|
| `analyze_intent` | 발화 성격(plan/followup)을 가리고, 지역/일수/동행/취향을 추출 | user_input, messages | intent, preferences, missing |
| `answer_follow_up` | 앞 답변에 이어지는 질문에 조건을 다시 묻지 않고 바로 답변 | user_input, messages | answer |
...
graph TD
    ...
    A --> F{앞 답변에<br/>이어지는 질문인가?}
    F -->|예| G[answer_follow_up<br/>이어서 답하기]
    F -->|아니오| B{region·days<br/>모두 있는가?}
...
**`answer_follow_up` 을 따로 둔 이유**가 두 번째 핵심입니다.

처음에는 노드를 4개(`analyze_intent` / `ask_clarify` / `recommend` / `build_itinerary`)만 두었습니다.
그랬더니 대화가 이렇게 끊겼습니다.

| 턴 | 입력 | 그때의 답변 |
|---|---|---|
| 1 | 마쓰야마 여행 코스 추천해줘 | (일정표 정상 출력) |
| 2 | 마쓰야마성은 어떻게 가는데? | "여행 가실 지역과 며칠 정도 다녀올 계획인지 알려주시겠어요?" |

이전 대화를 프롬프트에 넣어줘도 소용이 없었습니다. **모든 턴이 "새로 일정을 짜는 요청"이라는
전제로 흐르는 그래프**였기 때문입니다. 2턴처럼 이어지는 질문은 `days`가 없으니 `missing`이 차고,
그대로 되묻기로 빠집니다.

그래서 `analyze_intent`가 조건 추출과 함께 **발화의 성격(`intent`)까지 판정**하게 하고,
`followup`이면 추출·되묻기를 통째로 건너뛰고 `answer_follow_up`으로 보냈습니다.
이 노드는 이전 대화를 그대로 읽고 답만 합니다.

- 판정은 LLM이 합니다. "이어지는 질문인가"는 코드로 규칙을 짤 수 없는 판단이라서입니다.
- 대신 **첫 턴에는 `messages`가 비어 있으므로 코드에서 followup을 막습니다.** 모델이 잘못
  `followup`이라고 답해도 되묻기/추천 경로로 흐릅니다. LLM 판단은 코드로 한 번 더 거릅니다.
...
이 노드는 `history=state["messages"]` 를 함께 넘깁니다. 되묻기 다음 턴에 사용자가 "부산 2박 3일"
한 마디만 해도, 이전 대화가 프롬프트에 같이 들어가 있어서 지역·일수가 한 번에 채워집니다.
그래서 "앞선 대화에서 이미 나온 값은 유지하라"는 문장을 프롬프트에 넣었습니다.
다만 "그럼 후쿠오카로 바꿔줘" 같은 말에 옛 지역이 남는 문제가 생겨서
"이번 발화에 새 값이 나오면 덮어쓴다"는 문장을 한 줄 더 붙였습니다.

`intent` 를 같은 호출에서 함께 받는 이유는 **LLM 호출을 늘리지 않기 위해서**입니다.
분류용 노드를 따로 두면 매 턴 호출이 한 번 더 늘어나는데, 어차피 같은 대화를 읽고 판단하는 일이라
추출과 한 번에 시켰습니다.
...
### 2.5 answer_follow_up — 이어지는 질문에 답하기

**최종 System Prompt**

```
당신은 방금 여행 일정을 안내해 드린 여행 상담사입니다.
...
이 노드는 조건을 추출하지도, 후보를 다시 고르지도 않습니다. **이전 대화 + 이번 질문**만 보고 답합니다.
"조건을 다시 묻지 마세요"를 명시한 이유는, 프롬프트에 여행 상담사 역할을 주면 모델이 습관적으로
"어느 지역으로 가시나요?"를 되묻기 때문입니다.

"확실하지 않은 정보는 지어내지 말라"는 문장은 이 노드에서 특히 중요합니다. 교통편·영업시간 질문이
많이 들어오는데, 검색 없이 답하는 구조라 모델이 그럴듯한 시간표를 만들어냅니다.

**개선 전 → 후**

| 구분 | 내용 |
|---|---|
| 개선 전 | 이 노드가 아예 없었습니다. 모든 턴이 `analyze_intent` → 추출 → 되묻기/추천으로 흘렀습니다. |
| 그때의 문제 | "마쓰야마성은 어떻게 가는데?" 에 "여행 가실 지역과 며칠 일정인지 알려주시겠어요?" 라고 되물었습니다. 일정을 방금 안내해 놓고 다시 처음부터 묻는 셈이라 대화가 끊겼습니다. |
| 개선 후 | `intent` 판정과 `answer_follow_up` 노드를 추가해, 이어지는 질문은 추출·되묻기를 건너뛰게 했습니다. |
| 달라진 점 | "마쓰야마성은 시내에서 로프웨이로 5분이면 올라갑니다. 운행 시간은 현지에서 확인해 주세요." 처럼 대화가 이어집니다. |
...
> **`teams/team0/screenshots/` 폴더가 캡처를 넣으라고 미리 만들어 둔 자리입니다.**
> 여기에 `case1.png`, `case2.png`, `case3.png` 로 저장하세요.
...
| 입력 | 출력 (요약) | 실행 노드 |
|---|---|---|
| (1턴) 마쓰야마 2박 3일 코스 추천해줘 → (2턴) 마쓰야마성은 어떻게 가는데? | 2턴에서 `intent: "followup"` 으로 판정 → 조건을 되묻지 않고 이전 답변에 이어서 교통편 안내 | 1턴 `analyze_intent → recommend → build_itinerary` / 2턴 `analyze_intent → answer_follow_up` |

```markdown
![케이스 3 — 이어지는 질문](screenshots/case3.png)
```
...
- **잘 된 점**: 판단(코드)과 생성(LLM)을 분리해서 같은 입력이 항상 같은 경로로 흐릅니다.
  ...
  이어지는 질문을 별도 경로로 뺀 덕분에, 한 번 답한 뒤 추가 질문이 자연스럽게 이어집니다.
- **한계**:
  - 실제 여행 정보를 검색하지 않고 모델의 사전 지식에만 의존합니다. 영업시간·휴무일이 부정확할 수 있습니다.
  - 대화가 길어지면 최근 6개만 프롬프트에 넣습니다. 그보다 앞에서 말한 조건은 잊습니다.
    `preferences`를 State에 누적해두지 않고 턴마다 다시 뽑기 때문입니다.
  - `intent` 판정을 LLM이 하므로 경계가 애매한 발화("거기 대신 다른 데 추천해줘")는
    턴마다 다르게 흐를 수 있습니다. 첫 턴 오판만 코드로 막아두었습니다.
  - 예산·이동수단은 추출하지 않습니다.
- **다음에 개선한다면**:
  1. `preferences`를 턴 간에 누적 병합해서, 최근 6개 밖으로 밀려난 조건도 유지하기
...
> 문서를 어떻게 마무리했는지 남기는 항목입니다. 조원 전원이 모여서 정리한 기록을 적어주세요.
> (이 문서는 예제라 이름은 가상입니다)

| 항목 | 내용 |
|---|---|
| 회의 일시 | 2026-09-12 (금) 19:00 ~ 20:30 |
| 회의 장소 | 강의장 3층 스터디룸 B (오프라인) |
| 참석자 | 김지훈, 이서연, 박민호, 최유진 — 4명 전원 참석 |
| 불참자 / 사유 | 없음 |
| 기록자 | 이서연 |
...
- 각자 맡은 노드의 프롬프트와 개선 전/후 기록을 화면에 띄워놓고 한 명씩 설명했습니다.
- 그 자리에서 예시 질문 3개를 직접 돌려보며 캡처를 다시 찍었습니다.
  (첫 캡처에는 실행 노드 배지가 잘려 있어 다시 찍었습니다)
- 1.4 다이어그램과 `workflow.py` 의 엣지를 한 줄씩 대조해 일치하는지 확인했습니다.
- 한계와 개선 아이디어는 각자 하나씩 적어와서 합친 뒤, 중복을 정리해 3개로 추렸습니다.
- 문서 전체를 소리 내어 읽으면서 표현을 맞추고, 마지막에 함께 커밋했습니다.
