...
## 진행 순서 (이 문서부터 보시면 됩니다)

리드에게 조 브랜치(`team1` ~ `team7`)를 배정받았다면, 아래 순서대로 진행하시면 됩니다.

| 순서 | 할 일 | 어디를 보면 되나 |
|---|---|---|
| ① | 저장소 클론 → **우리 조 브랜치로 이동** | 아래 [시작하기](#시작하기) 1번 |
| ② | 설치하고 화면 띄우기 | [시작하기](#시작하기) 2~5번 (막히면 [SETUP_GUIDE](docs/SETUP_GUIDE.md)) |
| ③ | 0조 예제로 감 잡기 | `teams/team0/workflow.py`, `teams/team0/README.md` |
| ④ | **조원이 모여 주제 정하기** → 회의 표를 `teams/teamN/README.md` 맨 위에 기록 | `teams/team0/README.md` 맨 위 |
| ⑤ | 개발 (`workflow.py`, `prompts.py`) | 아래 [개발 5단계](#개발-5단계) |
| ⑥ | 예시 질문 3개 동작 화면 캡처 → 산출물 문서 작성 | [제출물 3개](#제출물-3개) |
| ⑦ | **조원이 모여 문서 마무리** → 산출물 정리 회의 표 작성 후 push | `teams/teamN/README.md` 맨 끝 |
...
대화 제목 옆의 **새 대화** 버튼을 누르면 그 조의 대화가 비워집니다.
이전 대화가 남아 있으면 다음 질문의 결과가 달라지므로, 첫 턴부터 다시 확인할 때 쓰세요.
