...
│  teams/team7/ 폴더 밖의 파일은 절대 수정하지 마세요.      │
│  (다른 조와 충돌이 납니다)                                  │
└────────────────────────────────────────────────────────────┘
...
작업 순서
  ...
  6. python run.py 와 npm run dev 로 화면을 띄워 예시 질문이 동작하는지 확인한 뒤 커밋하세요.

막히면 teams/team0/workflow.py (여행지 추천 예제) 를 열어보세요.

터미널에서 이 파일만 바로 실행해 볼 수 있습니다. (화면 없이 디버깅할 때 편합니다)

    python teams/team7/workflow.py "테스트할 질문"
"""
...
from teams.team7.prompts import FINAL_NODE_SYSTEM, FIRST_NODE_SYSTEM

# ── 1단계: 우리 조 정보 ──────────────────────────────────────────
# 화면의 탭 이름과 예시 질문으로 쓰입니다. 여기에 작성하시면 됩니다.
TEAM_INFO = {
    "name": "7조",       # 예: "7조 · 레시피 추천 봇"
