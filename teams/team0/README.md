...
| 입력 | 출력 (요약) |
|---|---|
| 친구랑 부산 2박 3일 가려고 하는데 맛집 위주로 추천해줘 | `analyze_intent`가 `{region: "부산", days: 2, companion: "친구", style: ["맛집"]}`를 추출 → `missing`이 비어 추천 경로로 분기 → 후보 3곳(돼지국밥 골목, 광안리 해변, 전포 카페거리) → Day 1/Day 2 일정표 + 준비물 2가지 |

실행 노드: `analyze_intent → recommend → build_itinerary` (화면 하단 배지로 확인 가능)

**예시 2 — 실패 / 엣지 케이스 (필수)**

| 입력 | 출력 | 왜 이렇게 나왔는지 |
|---|---|---|
| 여행 가고 싶어 | "어느 지역으로 가고 싶으세요? 며칠 일정인지도 알려주시면 좋아요. (예: 부산 2박 3일)" | 지역·일수가 모두 없어 `missing = ["region", "days"]` → `ask_clarify` 분기. 개선 전에는 모델이 지역을 임의로 지어내 바로 추천으로 넘어갔던 케이스입니다. |
| 아무데나 3일 | 되묻기 (지역만 재질문) | `days`는 3으로 추출되지만 `region`이 null이라 여전히 되묻기로 갑니다. "아무데나"를 지역으로 오인하지 않게 한 결과입니다. |

실행 노드: `analyze_intent → ask_clarify`
...
| 이름 | 맡은 부분 |
|---|---|
| (예제이므로 해당 없음) | 각 조는 이 표에 실제 담당자와 담당 노드를 적어주세요 |
...
- [x] 주제 선정 회의 표 작성 (일시 / 장소 / 참석자)
- [x] 화면에서 실제로 대화가 되는 것 확인
- [x] 1.1 ~ 1.3 표 작성
- [x] 1.4 mermaid 다이어그램이 실제 코드와 일치
- [x] 2장에 노드별 프롬프트 전문 + 개선 전/후 1회 이상
- [x] 3.1 실행 예시 2건 (실패 케이스 포함)
- [x] `git diff --name-only` 로 `teams/team0/` 밖 파일을 건드리지 않았는지 확인
